(function($){
        ...
        let options = {
            ...
            stop: function( event, ui ) {
                ui.helper.fadeOut(function(){
                    $(this).remove();renderWidgets();rend();//init();
                });
    ...
    function renderWidgets(initialize = false) {
        $('#col-list').html('');
        ...
        if(initialize)
            ...
            $('#col-list').addClass('disabled');
    }
    $(document).ready(function(){
        $('.dragable').css('height', (650)+'px');
        $( "#element" ).droppable({
            drop: function( event, ui ) {
                noOfColumn = (columns.map(c => c.toLowerCase()).indexOf(ui.draggable[0].id.split('-')[0]) + 1);
                if(noOfColumn > 0){
                    let i = 0;while(i < noOfColumn){addColumn();i++;};
                    arrangeItem();
                }
            }
        });
